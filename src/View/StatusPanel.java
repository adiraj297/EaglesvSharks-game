	...
	public StatusPanel(GameController2 controller, Board2 board3) {
		...
		for(int i = 0; i< board3.getBoard().length;i++)
		{
			for (int j = 0; j < board3.getBoard()[0].length; j++) {
				if (null != board3.getBoard()[i][j].getPiece() && !board3.getBoard()[i][j].getPiece().iconName().contains("obstacle")) {
					iconLabels.add(new JLabel(board3.getBoard()[i][j].getPiece().iconName()+ "\n Attack: "+ board3.getBoard()[i][j].getPiece().getAttackPower()+"\n Defence : "+ board3.getBoard()[i][j].getPiece().getDefencePower()+"\n Move : "+ board3.getBoard()[i][j].getPiece().getMovePower()));
				}
			}
		}
	...
	public StatusPanel(GameController3 controller, Board3 board4) {
		...
		for(int i = 0; i< board4.getBoard().length;i++)
		{
			for (int j = 0; j < board4.getBoard()[0].length; j++) {
				if (null != board4.getBoard()[i][j].getPiece() && !board4.getBoard()[i][j].getPiece().iconName().contains("obstacle")) {
					iconLabels.add(new JLabel(board4.getBoard()[i][j].getPiece().iconName()+ "\n Attack: "+ board4.getBoard()[i][j].getPiece().getAttackPower()+"\n Defence : "+ board4.getBoard()[i][j].getPiece().getDefencePower()+"\n Move : "+ board4.getBoard()[i][j].getPiece().getMovePower()));
				}
			}
		}
